...
func removeAtIndex(arr []int, index int) []int {
	if index < 0 || index >= len(arr) {
		// Return a copy of the original array if index is out of bounds
		return append([]int(nil), arr...)
	}
	// Create a new slice to ensure the original is not modified
	return append(append([]int(nil), arr[:index]...), arr[index+1:]...)
}

// A safe record is now also possible if removing one of the elements makes the record safe
func isSafe(record []int) bool {
	// If it's already safe, return
	if isSafeContinous(record) {
		return true
	}

	for i := 0; i < len(record); i++ {
		dampenedRecord := removeAtIndex(record, i)
		if isSafeContinous(dampenedRecord) {
			return true
		}
	}
	return false
...
func isSafeContinous(record []int) bool {
